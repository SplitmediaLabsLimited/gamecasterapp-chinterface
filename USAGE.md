...
This interface connects to Mixer's chat socket server. New messages arrive instantly as they are sent on Mixer.
...
async function mixer() {
    ...
    try {
        ...
        // To receive and listen for new, formatted whispers.
        mixer.on('whisper', data => {
            /**
             * `data` will always return the following structure:
             *
             * {string} id - The chat message ID; generated by Mixer.
